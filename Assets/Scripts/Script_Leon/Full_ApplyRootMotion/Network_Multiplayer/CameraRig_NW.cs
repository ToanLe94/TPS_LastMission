...
[ExecuteInEditMode]
public class CameraRig_NW : MonoBehaviour {

    public Transform target;
    public bool autoTargetPlayer;
    public LayerMask wallLayer;

    //public static System.Action<GameObject> onTargetLocalPlayerObj;
    //private GameObject targetLocalPlayerObj;
    ...
    //public void Awake()
    //{
    //    onTargetLocalPlayerObj += OnTargetLocalPlayerObj;
    //}

    //private void OnTargetLocalPlayerObj(GameObject obj)
    //{
    //    targetLocalPlayerObj = obj;
    //}
    ...
    void TargetPlayer()
    {
        if (autoTargetPlayer)
        {
            //if (targetLocalPlayerObj != null)
            //{
            //    Transform playerT = targetLocalPlayerObj.transform;
            //    target = playerT;

            //}
            if (Player_NW.localPlayerObj!= null)
            {
                Transform playerT = Player_NW.localPlayerObj.transform;
                target = playerT;
    ...
    {
        Debug.Log("Tham so cua AImView_ToShot"+ isZooming);

        if (!maincamera2)
        {
            Debug.Log("Camera la null nen ko vao duoc");
            return;
        }
        ...
        if (isZooming)
        {
            Debug.Log("vao duoc cho aim view");
            ...
            Debug.Log("cameraSettings.viewAim_Tranform.localPosition : "+ cameraSettings.viewAim_Tranform.localPosition);

            Debug.Log(" localpos camera :"+ maincamera2.transform.localPosition + "  newPos : "+ newpos);
