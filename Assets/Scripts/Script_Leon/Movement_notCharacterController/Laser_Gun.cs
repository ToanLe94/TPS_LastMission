            ...
            if (Input.GetKey(KeyCode.Mouse0) == true)
            {
                if (shootHit.transform.tag != "Effect_Gunshot")
                {
                    if (_isPlay_effect == false)
                    {
                        ...
                        var effect_Impact = Instantiate(Effect_Impact, shootHit.point, Quaternion.LookRotation(shootHit.normal));
                        //effect_Impact.GetComponent<ParticleSystem>().Play();
                        var PS_effect_Impact = effect_Impact.GetComponent<ParticleSystem>();
                        PS_effect_Impact.Play();
                        if (PS_effect_Impact != null)
                        {
                            Destroy(effect_Impact, PS_effect_Impact.main.duration);
